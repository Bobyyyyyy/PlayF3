...
import FormLinkGame from "./FormLinkGame";
import FormRandGame from "./FormRandGame";
import FormEnterGame from "./FormEnterGame";

function FormTrainingOption() {
    ...
    return (
        ...
        <div className={"w-full max-w-screen-sm md:max-w-3xl min-w-fit h-fit bg-secondary p-4 rounded-lg drop-shadow-2xl md:mt-8 border-4 border-primary mx-6 md:mx-0"}>
            <div className={"flex flex-col items-center justify-around gap-4"}>
                ...
                <div className="w-4/5">
                    {gameOption === "Gioca in locale" && <FormLocalGame />}
                    {gameOption === "Invita un amico" && <FormLinkGame />}
                    {gameOption === "Entra in una partita" && <FormEnterGame />}
                    {gameOption === "Cerca un avversario" && <FormRandGame />}
                </div>
            </div>
        </div>
...
export default FormTrainingOption;