...
export default function DailyChallenge() {
    ...
    let testo = (
        <>
            <h1 className="text-2xl font-bold md:text-4xl md:font-extrabold uppercase text-center mb-4">Daily Board</h1>
            ...
            <p className="mt-4 text-base font-normal md:text-xl md:font-medium">
                Ogni <b>giorno</b> ti aspetta un setup completamente nuovo sulla scacchiera,
    ...
    const fetchData = async () => {
        setIsLoading(true);
        let resPause = await isGamePaused("DAILY");
        if (!!resPause) {
            setPause(true);
            setDailyBoard(resPause);
        } else {
            let dailyConfigRes = await getBoardConfig("daily");
            setDailyBoard(dailyConfigRes);
            setPause(false);
        }
        setIsLoading(false);
    }
            ...
            <div className="flex flex-col justify-start w-fit items-center h-full md:mt-8 gap-8">
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col justify-evenly items-center min-h-fit h-full gap-y-10">
                        <BoardNotUsable position={dailyBoard} />
                        <Link
