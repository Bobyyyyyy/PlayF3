...
import CopyLink from "../../components/multiplayer/CopyLink.jsx";
...
export default function MultiPlayerGamePage() {
    ...
    //stati del gioco
    //const [, setPlayWhite] = useState(true);
    ...
    } = useTimer({ expiryTimestamp: expiryTimestampPlayer, onExpire: () => playerTimeout() });
    const {
    ...
    } = useTimer({ expiryTimestamp: expiryTimestampOpponent, onExpire: () => opponentTimeout() });
    ...
    function createWaitingSocketWhite() {
        fetch(Server.url + `/match/multiplayer`, {
            ...
            if (Object.keys(json).length !== 0) {
                ...
                setGameLink(window.location.href + "?match=" + json.match_shadow);
            }
        });
    }
    ...
    useEffect(() => {
        ...
        if (typeof query.match !== 'undefined') {
            match.white = false;
            match.setColor('b');
            black_match_shadow = query.match;
            setOpponentConnected(true);
        }
    ...
    return (
        <>
            {!opponentConnected ? (
                <CopyLink copyButton={copyButton} copyText={copyText} gameLink={gameLink} resign={resign} />
            ) : (
